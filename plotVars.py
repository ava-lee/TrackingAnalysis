...
def plotHisto(dataFrames, varName, xMin, xMax, xBins, varLabel, yLabel):
    ...
    if 'jet_trk' in varName: ax1.set_ylim([0, max(yMaxs)*1.4])
    ...
    if 'jet_trk' in varName: loc="best"
    else: loc="upper center"
    ax1.legend(loc=loc, labelspacing=0.3, handles=new_handles, labels=labels, facecolor='none', edgecolor='none')
